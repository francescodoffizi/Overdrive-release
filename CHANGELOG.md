...
### Fixed
- **Fix Freeze QCarCam ISP a 10-20s & Modalità MVC su DiLink 5.0 (`hook_qcarcam.cpp`, `DiLink5QCarCamBackend.java`)**:
  - **Root Cause #5 — Reset AIS da `qcarcam_s_param` FRAME_RATE (`hook_qcarcam.cpp`)**: Il nostro hook `qcarcam_start` chiamava `real_s_param(hndl, 7, fps_param)` per forzare 30 FPS, ma l'AIS service restituiva `error 20` (parametro non supportato in questo stato). Questo fallimento scatenava un reset asincrono della sessione AIS ~10 secondi dopo l'avvio, causando il freeze totale delle 4 camere tra il report 10s (ok 9.9 FPS) e il report 20s (freeze 0.0 FPS). **Fix**: rimossa completamente la chiamata `qcarcam_s_param` dall'hook `qcarcam_start`.
  - **Root Cause #6 — Modalità MVC AIS quando AVM ha il routing camera attivo (`DiLink5QCarCamBackend.java`)**: Quando `com.ts.avm.AvmAndroidService` (AVM HAL) mantiene sessioni camera attive, l'AIS service risponde ad aperture successive con il protocollo MVC (multi-view capture: 1 handle con 4 queue `qid=0-3`, invece di 4 handle separati). In modalità MVC, le telecamere rimangono in stato `init` indefinito perché `qcarcam_test` non inizializza correttamente i buffer per queue. **Fix**: aggiunto il flag `-noDisplay` al comando di avvio `qcarcam_test`, che forza il percorso `noDisplay` dell'AIS client (allocazione ION diretta per 4 handle separati, compatibile con l'hook) anche quando AVM è attivo.
  - **Root Cause #7 — Mutex Deadlock in `get_cam_vaddr` → DMA Streamer bloccato, `QCarCamBridge: Sidecar disconnected` (`hook_qcarcam.cpp`)**: Il `dma_streamer_thread` acquisiva `g_mutex` con `pthread_mutex_lock` all'inizio del ciclo di invio, poi chiamava `get_cam_vaddr()` che tentava di acquisire di nuovo lo stesso `g_mutex` non-ricorsivo → deadlock permanente → il thread restava bloccato nella syscall futex e non inviava mai dati → `byd_cam_daemon` leggeva zero byte dall'header del socket `@dilink5_cam` → `QCarCamBridge: Sidecar disconnected (header read failed)` ogni secondo. Nonostante le 4 telecamere ricevessero i primi frame (`[0:0] First Frame buf_idx 0 after : 222ms`), nessun frame arrivava all'app. **Fix**: rimosso il lock da `get_cam_vaddr()` — `g_cam_buffers` è scritto una sola volta durante `qcarcam_s_buffers` all'init, prima che arrivino i frame, quindi è stabile in lettura senza mutex. **Risultato**: tutte e 4 le camere stabili in stato `ok` per 80+ secondi consecutivi, DMA streamer emette 2.5–3.2 FPS (sentry mode), WebSocket streaming attivo (91 frames inviati a client web).
  - **Nota Architetturale — AIS Bloccato da Kill Ripetuti**: Se `qcarcam_test` viene terminato con SIGKILL mentre le telecamere sono in stato `freeze` (sessione AIS parzialmente attiva), l'ISP routing dell'AIS service può rimanere corrupto e impedire la delivery di frame a qualsiasi client successivo. L'unica recovery è il reboot del dispositivo. **Mitigazione futura**: implementare un watchdog che rilevi lo stato `freeze` prolungato (>30s) e esegua `qcarcam_stop`/`qcarcam_close` puliti prima di terminare il processo.


- **Ottimizzazione Reattività Dashboard in Tempo Reale & Prevenzione Sfarfallio "Dati non disponibili" (`DashboardFragment.kt`, `DashboardStatusParser.kt`)**:
  - **Frequenza di Aggiornamento Dinamica a 2 Secondi**: Ridotto l'intervallo di polling della dashboard da 15 secondi a 2 secondi quando l'app è in primo piano (`STATUS_REFRESH_ACTIVE_MS = 2_000L`), rendendo istantaneo l'aggiornamento della marcia (D/R/N/P), della velocità e dello stato di guida.
  - **Grace Period Anti-Flickering**: Aggiunto un meccanismo di tolleranza a 3 tentativi consecutivi prima di impostare lo stato `Unavailable`, evitando che micro-ritardi di rete facciano comparire transitoriamente la scritta "Dati veicolo non disponibili".
  - **Caching Asincrono della Stima Autonomia (`fetchPersonalizedRange`)**: Memorizzato in cache a 60 secondi il risultato di `/api/trips/range`, azzerando la latenza di lettura di `/status` ad ogni tick.
- **Risoluzione Rilevamento Marcia Reale (D/R/N/P) su DiLink 5.0 in modalità app_process (`GearMonitor.java`, `BydDataCollector.java`, `TelemetryDataCollector.java`)**:
  - **Supporto Proprietà HAL Dinamica (`0x21406407 SHIFT_MODE`)**: Aggiunto il fallback resiliente tramite `CarPropertyBridge` e `dumpsys car_service` per la lettura in tempo reale della proprietà HAL `0x21406407 (SHIFT_MODE)` e `0x21403a06 (GEAR_BOX_AUTO_MODE_TYPE_TWO)`. In precedenza, quando il demone girava come processo standalone (`app_process` UID 2000), il binding AIDL a `CarAdapterService` veniva rifiutato da AMS (`SecurityException: Unable to find app for caller`), causando il blocco permanente della marcia su `GEAR_P` e la visualizzazione dello stato "Pronta / Parcheggiata" anche con cambio in `(D)`.
  - **Aggiornamento Istantaneo Dashboard & Trigger Registrazione**: Sia `GearMonitor` che `TelemetryDataCollector` e `BydDataCollector` ora acquisiscono istantaneamente lo stato `D` (4), `R` (2), `N` (3), `P` (1), aggiornando la dashboard in tempo reale ("In Guida (D)") e attivando la registrazione continua.

- **Risolto mancato avvio stream video (Schermata Nera Web) su DiLink 5.0 (`hook_qcarcam.cpp`)**: 
  - Allargata la condizione di iniezione dell'hook nativo al processo `byd_cam_daemon` (oltre a `byd_apa`), permettendo la corretta creazione dei thread di streaming verso l'interfaccia web e la registrazione dashcam.


- **Fix Acquisizione Frame ISP in Modalità `-noDisplay` via `qcarcam_get_frame` ION mmap (`hook_qcarcam.cpp`)**:
  - **Root Cause #1**: Con il flag `-noDisplay`, `qcarcam_test` non chiama mai `test_util_post_window_buffer` → i frame counter rimanevano a 0 e lo streaming era a 0.0 FPS. **Fix**: hook diretto su `qcarcam_get_frame`.
  - **Root Cause #2**: `p_buf` in `qcarcam_plane_t` è un ION file descriptor (intero piccolo tipo `0x8`), non un virtual address. **Fix**: `mmap(ion_fd, PROT_READ, MAP_SHARED)` → VA reale (es. `0x70d9af0000`).
  - **Root Cause #3**: Doppio `qcarcam_release_frame` — il nostro hook lo chiamava esplicitamente, ma `qcarcam_test` lo chiama già internamente → ring buffer ISP corrotto dopo ~10s con freeze totale. **Fix**: rimosso il release esplicito dall'hook; aggiornamento solo atomico di `g_latest_buf_idx`.
  - **Root Cause #4**: Contesa ISP — `byd_cam_daemon` (watchdog) rilanciato automaticamente competeva sulle stesse risorse hardware. **Fix**: disabilitazione sentinel prima del test, watchdog rienabled post-verifica.
  - **Risultato Finale**: **4 camere a 30.0 FPS stabili per 40+ secondi** senza freeze (`cam0_frames=2350` in 40s), verificato con FPS report ogni 10s: `29.9 | 30.0 | 30.0 | 30.0` → `30.1 | 30.0 | 29.9 | 29.9` → `30.0 | 29.9 | 30.1 | 30.1` → `29.9 | 30.0 | 29.9 | 29.9`.

- **Risoluzione Blocco Fotogrammi all'Avvio & Rilascio Ring Buffer ISP QCarCam (`hook_qcarcam.cpp`)**:
  - **Rilascio Ciclico Buffer Hardware (`qcarcam_release_frame`)**: Ripristinato il rilascio immediato del buffer ISP in `_Z28test_util_post_window_buffer` associando ciascuna finestra `window` al rispettivo handle `qcarcam_hndl_t`. In precedenza, l'intercettazione del display blitting restituiva `0` senza rilasciare il buffer, esaurendo i 5 ring buffer hardware in 166 ms e bloccando l'emissione di nuovi frame da parte dell'ISP.
  - **Sincronizzazione Dinamica Ultimo Frame Valido (`g_latest_buf_idx`)**: Modificato `dma_streamer_thread` per comporre il mosaico e inviare i fotogrammi attingendo all'ultimo indice di buffer effettivamente popolato dall'hardware (`g_latest_buf_idx[cam]`) anziché ciclare staticamente su `s_idx % 5`.
  - **Preservazione Carico CPU (<5%)**: Mantenuto il bypass del rendering a display (`SurfaceComposerClient`), garantendo streaming continuo e live a 30 FPS con carico CPU quasi nullo.
...
- **Risoluzione Definitiva Marcia Dinamica (D/R/N/P) su DiLink 5.0 (`TelemetryDataCollector.java`, `GearMonitor.java`, `BydDataCollector.java`)**:
  - **Integrazione Primaria `CarBodyManager.getShiftMode()` in `TelemetryDataCollector`**: Sostituita l'interrogazione esclusiva di `BYDAutoGearboxDevice` (che su DiLink 5.0 / Sealion 7 restituiva staticamente marcia `P` mascherando i cambi di marcia reali) con l'adattatore `"body"` (`CarBodyManager.getShiftMode()`), mappando in tempo reale le marce `1=P`, `2=R`, `3=N`, `4=D` nel fast-path di telemetria a 5 Hz.
  - **Inversione Priorità di Risoluzione in `GearMonitor`**: Impostata come prima sorgente `readFromCarAdapter()` (`CarBodyManager.getShiftMode()`) nel loop di polling a 200 ms, impedendo a snapshot HAL obsoleti di mantenere il veicolo bloccato su `GEAR_P` da fermo.
  - **Priorità Adattatore Vettura in `BydDataCollector.readGearNow()`**: Assegnata priorità immediata a `readGearFromCarAdapter()` rispetto al vecchio getter `getGearboxAutoModeType()`, garantendo coerenza di stato su tutta la telemetria di bordo e i logger di viaggio (`TripDetector`).
