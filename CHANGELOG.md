...
- **Hardening Superfici & Overlay, Risoluzione Crash Adreno SA8155P, Status Overlay FGS e Prevenzione Caduta Wi-Fi (`SafeOverlayHelper.kt`, `MessageOverlayService.java`, `StatusOverlayService.java`, `AndroidManifest.xml`, `RoadSenseOverlayService.kt`, `RemoteVoiceService.java`, `NavPromptOverlay.kt`, `BootReceiver.kt`, `AccMonitorController.kt`, `AccMonitor.java`)**:
  - **Modulo Centralizzato `SafeOverlayHelper.kt`**: Unificato il ciclo di vita e la sanitizzazione di tutti i 5 servizi overlay dell'app. Rimozione forzata del flag `FLAG_HARDWARE_ACCELERATED` e applicazione di `View.LAYER_TYPE_SOFTWARE` su tutte le root view delle finestre overlay, impedendo a monte la generazione di contesti EGL hardware concorrenti in SurfaceFlinger.
  - **Risoluzione Crash SurfaceFlinger / Adreno 640 (`MessageOverlayService.java`)**: Risolto il `SIGSEGV` in `libadreno_utils.so` (`validate_resource_memory_layout_metadata`) che si verificava all'invio di messaggi overlay/test message mentre `byd_cam_daemon` gestisce attivamente buffer DMA-BUF/GraphicBuffer. Aggiunto guard con `ProjectionStateMonitor` per sopprimere overlay se telecamera 360 BYD (`com.byd.panoramic`) o CarPlay sono in primo piano.
  - **Allineamento Permessi Foreground Service (`StatusOverlayService.java`, `AndroidManifest.xml`)**: Dichiarato `android:foregroundServiceType="specialUse|dataSync"` nel Manifest per allinearlo con la richiesta runtime su Android 10 (API 29 `FOREGROUND_SERVICE_TYPE_DATA_SYNC`), eliminando il fallimento di `startForeground` e ripristinando la corretta visualizzazione della pillola di stato overlay (REC / TRIP / MIC). Applicato rendering software anche ai pulsanti di chiusura ✕.
  - **Hardening di Tutti gli Overlay (`RoadSenseOverlayService.kt`, `RemoteVoiceService.java`, `NavPromptOverlay.kt`)**: Convertite tutte le finestre traslucide e pillole al rendering software protetto da `SafeOverlayHelper`.
  - **Watchdog Wi-Fi Ridondante su Doppio Spegnimento ACC-OFF (`BootReceiver.kt`, `AccMonitorController.kt`, `AccMonitor.java`)**: Sostituito il singolo timer di re-arm con una sequenza a 3 step progressivi (2.5s, 3.5s, 5.0s, copertura ~11s) per contrastare in modo persistente le disattivazioni multiple della scheda Wi-Fi invocate dal demone BYD `CarPowerBinder: turnOffWifi` durante spegnimenti ravvicinati.
...
- **Connettività Resiliente & Failover su SIM Interna T-Box (`SingboxLauncher.kt`, `GlobalProxyDaemon.java`, `ProxyHelper.java`, `NetworkFailoverWatchdog.kt`, `DaemonStartupManager.kt`, `BootReceiver.kt`, `AccSentryDaemon.java`)**:
  - **Failover Puramente Software & Protezione Connessione Wi-Fi (`NetworkFailoverWatchdog.kt`)**: Rimosso qualsiasi comando distruttivo (`cmd wifi disconnect`); il failover del traffico Overdrive su `sing-box` (porta 8119 su SIM T-Box) avviene in modo trasparente a livello applicativo tramite `ProxyHelper`, preservando sempre l'associazione dell'interfaccia `wlan0` per garantire la raggiungibilità continua di ADB, Home Assistant e rete locale.
  - **Risoluzione Errore Kernel Linux `operation not permitted` (`SingboxLauncher.kt`, `GlobalProxyDaemon.java`)**: Rimossa la direttiva `"bind_interface": "vlan4"` dalla configurazione di `sing-box` che falliva sotto UID 2000 (shell) a causa della mancanza di `CAP_NET_RAW`.
  - **Instradamento Adattivo Wi-Fi / Proxy (`ProxyHelper.java`)**: In `ProxyHelper.getHttpProxy()`, priorità alla navigazione diretta su Wi-Fi quando la rete wireless è associata e validata su Internet (`isWifiHealthy()`); deviazione automatica e immediata su `127.0.0.1:8119` (proxy VLESS su SIM interna) non appena il Wi-Fi cade o è assente.
  - **Avvio Resiliente Daemon Proxy (`DaemonStartupManager.kt`)**: Avvio e supervisione automatica di `sing-box` all'accensione come componente essenziale della resilienza di rete, garantendo la disponibilità perenne del socket locale porta 8119 a tutti i componenti di Overdrive.
  - **Riarmo Wi-Fi Privilegiato su Spegnimento Veicolo (`AccSentryDaemon.java`, `BootReceiver.kt`)**: Intercettata la transizione a motore spento (ACC-OFF): contrastata la disattivazione forzata operata dal servizio di bordo BYD (`TsCarPower: turnOffWifi`) eseguendo il riarmo via shell con privilegi UID 2000 (`cmd wifi set-wifi-enabled enabled 2>/dev/null || svc wifi enable 2>/dev/null`) dopo 2.5 secondi, permettendo all'infotainment di riagganciarsi stabilmente al Wi-Fi domestico anche ad auto spenta/parcheggiata.
