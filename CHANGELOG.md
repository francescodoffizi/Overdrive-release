...
- **Connettività Resiliente & Failover su SIM Interna T-Box (`SingboxLauncher.kt`, `GlobalProxyDaemon.java`, `ProxyHelper.java`, `NetworkFailoverWatchdog.kt`, `DaemonStartupManager.kt`)**:
  - **Vincolo Outbound VLESS su `vlan4`**: Aggiunta la direttiva `"bind_interface": "vlan4"` nella configurazione di `sing-box` e la creazione proattiva della rotta statica host verso l'IP del server VLESS (`80.225.224.92/32`) attraverso il gateway Ethernet del T-Box (`172.16.*.101`). Questo garantisce a `sing-box` la capacità di uscire verso Internet attraverso la SIM cellulare BYD anche in assenza di default route Android.
  - **Instradamento Adattivo Wi-Fi / Proxy (`ProxyHelper.java`)**: In `ProxyHelper.getHttpProxy()`, priorità alla navigazione diretta su Wi-Fi quando la rete wireless è associata e validata su Internet (`isWifiHealthy()`); deviazione automatica e immediata su `127.0.0.1:8119` (proxy VLESS su SIM interna) non appena il Wi-Fi cade o diventa "zombie".
  - **Avvio Resiliente Daemon Proxy (`DaemonStartupManager.kt`)**: Avvio e supervisione automatica di `sing-box` all'accensione come componente essenziale della resilienza di rete, garantendo la disponibilità perenne del socket locale porta 8119 a tutti i componenti di Overdrive (Telegram Bot, Meteo, Sincronizzazione, Aggiornamenti, Mappe/Routing).
  - **Watchdog Asincrono Anti "Wi-Fi Zombie" (`NetworkFailoverWatchdog.kt`, `OverdriveApplication.kt`)**: Riattivato il watchdog di failover con avvio ritardato a 45 secondi dal boot (per proteggere l'avvio di `system_server`); rileva se l'hotspot wireless perde la connettività cellulare a monte e disconnette temporaneamente `wlan0` per non bloccare il traffico, invalidando la cache del proxy per un failover istantaneo.
- **Riarmo Wi-Fi su Spegnimento Veicolo (`AccMonitor.java`, `AccMonitorController.kt`)**:
  - Intercettata la transizione a motore spento (ACC-OFF): contrastata la disattivazione forzata operata dal servizio di bordo BYD (`TsCarPower: turnOffWifi`) schedulando con un ritardo di 2.5 secondi il riarmo via shell (`cmd wifi set-wifi-enabled enabled`), permettendo all'infotainment di riagganciarsi stabilmente al Wi-Fi domestico anche ad auto spenta/parcheggiata.
