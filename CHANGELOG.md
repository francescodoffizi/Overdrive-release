...
- **Integrazione Demone Nativo Zero-Copy `fast_cam_capture` & `libfast_cam_client.so` su DiLink 5.0 (`qcarcam_bridge.cpp`, `DiLink5QCarCamBackend.java`, `DiLink5PowerDiagnostics.java`, `CMakeLists.txt`)**:
  ...
  - **Integrazione Precompilata `fast_cam_client` (NDK/JNI)**: Inclusa la libreria client precompilata `libfast_cam_client.so` (`arm64-v8a`) e gli header nativi `fast_cam_bridge.h` e `fast_cam_ipc.h` collegati al modulo C++ `surveillance` in `CMakeLists.txt`. Rimossi i target obsoleti `hook_qcarcam` e `dilink5_cam_sidecar`.
  - **Aggiornamento Stream Bridge Nativo (`qcarcam_bridge.cpp`)**: Implementato il thread client basato sull'API RAII C++ `FastCamClient`, con connessione diretta al socket `/data/local/tmp/fast_cam.sock`, ricezione dei fotogrammi zero-copy (`FastCamFrame`, UYVY 1920x1300 a 30 FPS) tramite descrittori DMA condivisi e rendering ad alte prestazioni su `ANativeWindow` via SIMD ARM NEON.
  - **Supervisore Automatico & Auto-Deploy (`DiLink5QCarCamBackend.java`)**: Confezionato il binario standalone `fast_cam_capture` negli asset dell'applicazione (`assets/dilink5/fast_cam_capture`) con estrazione automatica all'avvio in `/data/local/tmp/fast_cam_capture` (permessi `755`). Il supervisore gestisce l'avvio con `export LD_LIBRARY_PATH=/vendor/lib64:/system/lib64:/data/local/tmp && exec /data/local/tmp/fast_cam_capture --all`, il kill preventivo dei processi obsoleti e il drain asincrono dei log.
